...
handlers.order.post = (data, callback)=>{
	...
	let cardDate = typeof(data.payload.cardDate) === 'string' && data.payload.cardNum.trim().length === 5 ? data.payload.cardDate : false;
	let cardCvc = typeof(data.payload.cardNum) === 'string' && data.payload.cardCvc.trim().length === 3 ? data.payload.cardCvc : false;
	if(email && cardNum){
		// Check the token
		let token = typeof(data.headers.token) === 'string' ? data.headers.token : false;
		handlers._tokens.verifyToken(token, email, (tokenIsValid)=>{
